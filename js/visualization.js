...
document.addEventListener('DOMContentLoaded', () => {
   ...
   /**
    ...
    // Create a force simulation with improved parameters and fallbacks
    if (simulation) {
        simulation.stop();
    }
    ...
    simulation = d3.forceSimulation(nodes)
        .force('link', d3.forceLink(links)
            .id(d => d.id)
            .distance(d => 100 + ((d.depth || 0) * 30))
        )
        .force('charge', d3.forceManyBody()
            .strength(d => -300 + ((d.depth || 0) * 50))
        )
    ...
    <div style="
        ...
        <div style="
            ...
            align-items: center;
            margin-bottom: 16px;
            background: linear-gradient(90deg, ${d.color}, #fff0);
            padding: 10px 16px;
            border-radius: 4px;
            ">
            <span style="
            ...
            <h1 style="margin: 0; font-size: 20px; font-weight: 600;">${d.name}</h1>
        </div>
        ...
        <div style="margin-top: 20px; font-size: 13px; color: #777; display: flex; align-items: center;">
